...
class SpeechDataset(Dataset):
    ...
    def prepare(self, listfile):
        ...
        if 'texts' in self.keys:
            ...
            texts, breaks, lengths, ph_per_word = self._text_processor(texts)
        else:
        ...
        self.metadata = filter(zip(texts, breaks, lengths, paths, stems, speakers, styles, ph_per_word))

        self.update(self.metadata[:])

    def update(self, metadata):
        self.texts, self.breaks, self.lengths, \
            self.paths, self.stems, self.speakers, self.styles, self.ph_per_word = zip(*metadata)
    ...
    def __getitem__(self, index):
        ...
        if 'texts' in self.keys:
            ...
            data['word_boundaries'] = np.array(self.ph_per_word[index])
