...
def AuraSTFT_metric(
    ...
    if reference.ndim == 3:
        ...
        estimate = estimate.permute(0, 2, 1)
...
def get_metrics(
    ...
    mix : np.ndarray, optional
        The mixed audio signal with shape (channels, length). Required for 'log_wmse' metric.
    ...
    -------
    List[Tuple[str, float]]
        A list of tuples containing (metric_name, metric_value).
    """
    result = []
