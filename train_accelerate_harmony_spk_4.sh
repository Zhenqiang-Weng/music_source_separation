...
PYTHONWARNINGS="ignore" accelerate launch --mixed_precision bf16 --num_processes 2 --main_process_port ${MASTER_PORT} train_accelerate_bf16.py \
    --model_type spk_bs_roformer_exportable \
    --config_path ckpt/single_stem/config.yaml \
    --start_check_point "results/results_spk_bs_roformer-with-large-window/new.ckpt" \
    ...
    --wandb_name "spk_bs_roformer--with-large-window-$(date +%Y%m%d_%H%M%S)" \
    --wandb_key "${WANDB_KEY:-}"