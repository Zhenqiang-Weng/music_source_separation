...
class MSSDatasetWithSpk(torch.utils.data.Dataset):
    def __init__(self, config, data_path, metadata_path="metadata.pkl", dataset_type=1, batch_size=None, verbose=True):
        ...
        self.mix_instruments = config.training.get('mix_instruments', self.instruments)
    ...
    def get_metadata(self):
        ...
        if self.dataset_type in [1, 4]:
            ...
            metadata = dict()
            for instr in self.mix_instruments:
                metadata[instr] = []
                track_paths = []
                if type(self.data_path) == list:
                    for tp in self.data_path:
    ...
    def load_source_from_path(self, track_path, instr, track_length=None):
        """从指定路径加载指定乐器的音轨
        
        Args:
            track_path: 音轨文件夹路径（dataset_type 1,4）或音频文件路径（dataset_type 2,3）
            instr: 乐器名称
            track_length: 音轨长度（可选，如果为None则自动获取）
            
        Returns:
            torch.Tensor: 形状为 (2, chunk_size) 的音频数据
        """
        should_print = (not dist.is_initialized() or dist.get_rank() == 0)
                ...
                if os.path.isfile(path_to_audio_file):
                    try:
                        if track_length is None:
                            track_length = sf.info(path_to_audio_file).frames
                        source = load_chunk(path_to_audio_file, track_length, self.chunk_size)
                        source = ensure_two_channels(source)
                    except Exception as e:
                        if should_print:
                            print('Error loading from path: {} Path: {}'.format(e, path_to_audio_file))
                        source = np.zeros((2, self.chunk_size), dtype=np.float32)
                    break
        else:
            # track_path 是具体音频文件路径
            # 需要根据 instr 构造正确的路径
            # 假设路径格式为: base_path/instr/file.wav
            base_path = os.path.dirname(track_path)
            file_name = os.path.basename(track_path)
            # 替换文件名中的乐器名称
            # 这里假设文件名包含乐器信息，可能需要根据实际情况调整
            path_to_audio_file = os.path.join(os.path.dirname(base_path), instr, file_name)
            
            if not os.path.isfile(path_to_audio_file):
                # 如果路径不存在，尝试其他可能的路径格式
                path_to_audio_file = track_path.replace(os.path.basename(os.path.dirname(track_path)), instr)
            
            try:
                if track_length is None and os.path.isfile(path_to_audio_file):
                    track_length = sf.info(path_to_audio_file).frames
                source = load_chunk(path_to_audio_file, track_length, self.chunk_size)
                source = ensure_two_channels(source)
            except Exception as e:
        ...
        # 不应用数据增强，因为这是用于加载额外音轨的
        source = ensure_two_channels(source)
        return torch.tensor(source, dtype=torch.float32)

    def load_random_mix(self):
        res = []
        track_paths = []  # 保存每个stem的track_path
        
        for instr in self.instruments:
            s1, track_path = self.load_source(self.metadata, instr)
        ...
        # 加载额外的混音音轨（使用第一个track_path作为基准）
        extra_sources = []
        if hasattr(self.config.training, 'mix_instruments') and len(track_paths) > 0:
            base_track_path = track_paths[0]  # 使用第一个音轨的路径作为基准
            for instr in self.config.training.mix_instruments:
                if instr not in self.instruments:
                    try:
                        extra_source = self.load_source_from_path(base_track_path, instr)
                        extra_sources.append(extra_source)
                    except Exception as e:
                        # 如果加载失败，跳过该音轨
                        should_print = (not dist.is_initialized() or dist.get_rank() == 0)
                        if should_print:
                            print(f'Failed to load extra instrument {instr}: {e}')
    
        return res, extra_sources, None
    ...
    def load_aligned_data(self):
        ...
        # Load extra sources aligned
        extra_sources = []
        if hasattr(self.config.training, 'mix_instruments'):
            for instr in self.config.training.mix_instruments:
                if instr not in self.instruments:
                    source = None
                    for extension in self.file_types:
                        ...
                        if os.path.isfile(path_to_audio_file):
                            try:
                                ...
                                if should_print:
                                    print('Error loading extra source: {} Path: {}'.format(e, path_to_audio_file))
                                source = np.zeros((2, self.chunk_size), dtype=np.float32)
                            break
                    if source is None:
                        source = np.zeros((2, self.chunk_size), dtype=np.float32)
                    extra_sources.append(torch.tensor(source, dtype=torch.float32))
    ...
    def __getitem__(self, index):
        if self.dataset_type in [1, 2, 3]:
            res, extra_sources, embedding = self.load_random_mix()
        else:
            res, extra_sources, embedding = self.load_aligned_data()
        
        # 合并所有音轨用于混音
        if len(extra_sources) > 0:
            extra_sources_stacked = torch.stack(extra_sources)
            all_sources = torch.cat([res, extra_sources_stacked], dim=0)
        else:
            all_sources = res
        
        
        if self.aug:
            if 'random_drop_stems' in self.config['augmentations']:
                drop_prob = self.config['augmentations']['random_drop_stems_prob']
                for id, instrument in enumerate(self.mix_instruments):
                    if instrument in self.config['augmentations']['random_drop_stems_instruments']:
                        if random.uniform(0, 1) < drop_prob:
                            all_sources[id] = torch.zeros_like(all_sources[id])
        ...
        res = all_sources[:len(res)]  # 提取要分离的音轨部分
        mix = all_sources.sum(0)  # 使用所有音轨生成混音
        ...
        if self.aug:
            ...
            if 'disharmony' in self.config['augmentations']:
                if self.config['augmentations']['disharmony']:
                    # Apply disharmony effect here
                    pass
        ...
        if self.config.training.target_instrument is not None:
            ...
            return res[index:index+1], mix, embedding
