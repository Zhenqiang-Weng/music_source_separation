...
class MSSDataset(torch.utils.data.Dataset):
    ...
    def load_aligned_data(self):
        ...
        return torch.tensor(res, dtype=torch.float32), extra_sources
    ...
    def __getitem__(self, index):
        if self.dataset_type in [1, 2, 3]:
            res, extra_sources = self.load_random_mix()
        else:
            res, extra_sources = self.load_aligned_data()
