...
def demix_with_spk(
    ...
    NOTE: generic mode only, no progress bar.
    """
...
def demix_with_spk_2(
    ...
    max_dist = getattr(clustering_result, "max_inter_cluster_distance", None)
    if (max_dist is None) or (max_dist <= 0.3):
        print(f"[demix_with_spk_2] Skip Guided Demix (max_inter_cluster_distance={max_dist})")
        return waveforms_blind
