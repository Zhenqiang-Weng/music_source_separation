...
def get_model_from_config(model_type: str, config_path: str) -> Tuple[nn.Module, Union[ConfigDict, OmegaConf]]:
    ...
    if model_type == 'mel_band_roformer':
        ...
        from models.bs_roformer import SpeakerBSRoformer
        model = SpeakerBSRoformer(**dict(config.model))
    elif model_type == 'band_split_roformer':
...
def setup_ddp(rank: int, world_size: int) -> None:
    ...
    initialize the process group with the NCCL backend (preferred for GPj Us),
